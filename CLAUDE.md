...
There is NO italic. The section intros were briefly Cormorant italic, and it was reverted: a real
italic is a second face, 39,304B of preloaded woff2, for three paragraphs, and it was the least
visible of the three changes it shipped with. Do not reintroduce it, and do not reach for
font-style: italic on Cormorant either: the served font has no italic, so the browser would
synthesise a slant. next/font requests weight only.
...
/terms carries the Circle terms in plain language, linked from the Circle block and the footer, and
it is INDEXED and in the sitemap. It is not boilerplate: cancel any time, no notice period, no exit
fee is what a hesitant buyer searches for before paying, and it is the page a model needs to answer
"can I cancel". /welcome stays noindex.
Nothing on that page may carry a commitment that was not agreed. The recordings line used to promise
deletion after 30 days, which nobody agreed to; it now says recorded for members only, matching the
Circle accordion. Before adding a line there, check it against the page.
...
## Machine readable
Four artifacts, all generated, none hand written, none carrying anything a reader cannot see.
- app/robots.ts. Wildcard Allow, then GPTBot, ClaudeBot, PerplexityBot, CCBot and Google-Extended
  named in a second group with the same Allow, so the position on AI crawlers is stated rather than
  defaulted. Host and Sitemap point at the www canonical. No Disallow anywhere: /welcome and /terms
...
- content/schema.ts. One @graph in one script tag, built from content/copy.ts so it cannot drift.
  ...
  carrying an Offer at 499 USD with a monthly UnitPriceSpecification and the Stripe URL, and a
  FAQPage built from the six accordion pairs verbatim. areaServed names the five markets the page
  names, typed Country, City and AdministrativeArea, with "Worldwide" kept as a sixth value rather
  than standing alone. No Review, no AggregateRating, no node describing anything off the page.
  Validated against schema.org via schema-dts in strict mode, which type checks every @type and
  every property against the full vocabulary. Clean.
- app/llms.txt/route.ts. Plain text at /llms.txt, force-static, generated from copy.ts. What the
  practice is, both offers with their terms and routes, the markets, the operated list, the contact
  routes. Facts only. Adding marketing language here defeats it.
The rendered page has zero parse5 errors. components/Logotype.tsx reads the SVG and injects nothing:
app/_brand/logotype.svg already carries role="img" and aria-label, and adding a second aria-label in
the component emitted a duplicate attribute. Label the mark in the SVG, never in the component.
