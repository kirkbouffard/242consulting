...
- app/sitemap.ts. Two entries, / and /terms. NOTE the conflict: /terms sets robots noindex in
  app/terms/page.tsx, so the sitemap submits a URL that will not be indexed. Harmless, not a bug to
  "fix" in passing, but pick one if it ever matters: drop the sitemap entry or drop the noindex.
...
Known, pre-existing, NOT from the SEO pass: the nav mark renders a duplicate aria-label, because
components/Logotype.tsx injects one and app/_brand/logotype.svg already carries one. Single parse5
error on the page. Fix it in Logotype.tsx or in the generated SVG, not both.
